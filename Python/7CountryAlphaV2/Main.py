...
def Multi_Country(S,I,sigma):
    ...
    UseStaggeredAges = False #Activates using staggered ages
    ...
    MortalityRates, Nhat_matrix, Nhat_ss = Stepfuncs.getDemographics(demog_params, UseStaggeredAges, UseDiffDemog, DemogGraphs, I_all, I_touse)
    ...
    wpath, rpath, Cpath, Kpath, Ypath = Stepfuncs.get_Timepath(tp_params, wpath_initguess, rpath_initguess, assets_init, kd_ss, kf_ss, w_ss, r_ss, PrintLoc, Print_cabqTimepaths, UseTape)
	
    if TPIGraphs==True:
            Stepfuncs.plotTimepaths(I, S, T, sigma, wpath, rpath, Cpath, Kpath, Ypath, I_touse,SAVE,SHOW)


Multi_Country(80,3,2)
