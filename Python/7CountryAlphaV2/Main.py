...
demog_params = (I, S, T, T_1, LeaveHouseAge, FirstFertilityAge, LastFertilityAge, FirstDyingAge, MaxImmigrantAge, g_A, demog_ss_tol)
MortalityRates, Nhat_matrix, KIDs, Nhat_ss, KIDs_ss, lbar = Stepfuncs.getDemographics(demog_params, PrintAges, DiffDemog, countrynames)

#Stepfuncs.plotDemographics((S,T),[0,1,2,3,4,5,6],[0],str("Initial"), Nhat_matrix, countrynames)
...
params_ss = (T, I, S, beta, sigma, delta, alpha, e, A, FirstFertilityAge, FirstDyingAge, Nhat_ss, MortalityRates[:,:,-1], g_A, chi, rho, KIDs_ss)
...
if CalcTPI==True: #Time Path Iteration, activated by line 24
	print "Beginning TPI"
	initialguess_params = (I, S, T, delta, alpha, e, A, FirstFertilityAge, FirstDyingAge, Nhat_matrix[:,:,0], MortalityRates[:,:,0], g_A)
	...
	tp_params = (I, S, T, T_1, beta, sigma, delta, alpha, e, A, FirstFertilityAge, FirstDyingAge, Nhat_matrix, MortalityRates, g_A, distance, diff, xi, MaxIters)
