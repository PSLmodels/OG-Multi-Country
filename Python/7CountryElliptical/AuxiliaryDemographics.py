...
def getkeyages(S, PrintAges):
    ...
    Objects in Function:
        agestopull        = (S) List, Contains the ages from the data files to use for the 
                            demographic data in the program
        LeaveHouseAge     = Int, Age where children become adults and are no longer dependent
                            on parents for consumption
    ...
    Returns: LeaveHouseAge, FirstFertilityAge, LastFertilityAge, MaxImmigrantAge,
    ...
    #agestopull = np.arange(100)[np.where(np.in1d(np.arange(100),\
     #       np.round(np.linspace(0, 100, num=S, endpoint=False))))]

    LeaveHouseAge = np.abs(agestopull - 21).argmin()#The age when agents become independent
                                                    #households and don't rely on 
                                                    #parents consumption
    FirstFertilityAge = np.abs(agestopull - 23).argmin()#The age when agents have their first 
                                                        #children
...
def plotDemographics(ages, datasets, I, S, T, I_touse, T_touse = None,\
    ...
    """
                ...
                plot2: Will show 2 different plots depending on the input value 
                       'compare_across'.
                         If compare_across == "T": plot2 will display a plot of Nhat for 
                                                  each year in the function input T_touse
                                                  with each country 
                         If compare_across == "I": plot2 will display a plot for each country
                                                   of Nhat in whatever year in input value 
                                                   'data_year' is

        Inputs:
            - ages                  = tuple: Contains FirstFertilityAge, LastFertilityAge, 
                                             FirstDyingAge, and MaxImmigrantAge from the 
                                             OLG class
            - datasets              = tuple: Contains the arrays FertilityRates, 
                                              MortalityRates, ImmigrationRates, 
                                              and Nhat from the OLG class
            ...
            - T_touse               = List: [Unknown], List of years in plot2 given from 
                                            user input
            - compare_across        = String: (Either "T" or "I"), changes the output of 
                                              plot2 (see function description)
            - data_year             = Int: The year plot1 will display for each countries 
                                           demographic data
        ...
        Variables Called from Object:
            ...
            - MaxImmigrantAge       = Int: No immigration takes place for cohorts older 
                                           than this age
            - FertilityRates        = Array: [I,S,T], Fertility rates for each country, 
                                             cohort, and year
            - MortalityRates        = Array: [I,S,T], Mortality rates for each country,
                                             cohort, and year
        ...
        Objects in Function:
            - subplotdim_dict       = Dictionary: [6], Contains keys for each int 2-7 
                                                  that maps to a dimensionality of the 
                                                  subplots of plot2
            - magic_int             = Int: Value from subplotdim_dict that indicates the 
                                           number and dimensionality of subplots of plot2
    ...
    #The firstPlot is a subplot of key demographic data and the population dynamics
    #for each country
    def firstPlot():
        plt.subplot(231)
        for i in range(I):
        ...
        for i in range(I):
            plt.plot(range(FirstFertilityAge,LastFertilityAge+1),\
                    FertilityRates[i,FirstFertilityAge:LastFertilityAge+1,data_year])
        ...
        for i in range(I):
            plt.plot(range(FirstFertilityAge,LastFertilityAge+LeaveHouseAge),\
                    Kids[i,FirstFertilityAge:LastFertilityAge+LeaveHouseAge,data_year])
        plt.xlabel('Age')
        plt.ylabel('Kids')
        ...
        #Transition path for total population of each country from the initial 
        #shares to the steady-state
        plt.subplot(236)           
        for i in range(I):
...
#The secondPlot compares population shares across years or across countries
#depending on the function input 'compare_across'
    def secondPlot():

        #Dictionary that contains ideal dimensions of the subplot depending on
        #how many countries are being used
