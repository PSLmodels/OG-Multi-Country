...
\section{Add a Leisure Decision}
	...
	As an alternative, we can use \texttt{fsolve} to find the whole histories of $\{r_t\}$ and $\{bq_{it}\}$. that satisfy equations \eqref{eq_3cond2} and \eqref{eq_2BQcomb} for all time periods.

	\begin{equation}
		bq_{it} - \frac{\sum_{s=23}^{67} \hat bq_{ist} (1-\rho_{ist}) \hat N_{ist}} {\sum_{s=67}^S \hat a_{ist} \rho_{ist} \hat N_{ist}} = 0 \label{eq_2BQcomb}
	\end{equation}
