...
\section{Adding Different Types of Labor}
	...
	\begin{align}
		...
		w_{ijt} & = (1-\alpha_j) \frac{\hat y_{it}}{\hat n_{ijt}}; \forall i.j \label{eq_5wfunc}
	\end{align}
	\begin{align}
	...
	Next, make a guess for the history of capital stocks and labor inputs, $\left\{ \hat k_{it}, \hat k^f_{it}, \hat n_{ijt} \right\}\; \forall i,j,t$.

	Equations \eqref{eq_5yfunc} - \eqref{eq_5wfunc}, then give us $\left\{ \hat y_{it}, r_{it}, \hat n_{ijt} \right\}\; \forall i,j,t$

	Equation \eqref{eq_5bqfunc} gives $\left\{ \hat{bq}_{it} \right\}\; \forall i,t$.

	Next, given the initial asset holdings (the $a$'s), we can use an fsolve along with equations \eqref{eq_5HH1func} - \eqref{eq_5HH5func} to find $\left\{ \hat c_{ijst}, \hat c^K_{ijst}, \hat \ell_{ijst}, \hat a_{ijst} \right\}\; \forall i,j,s,t$.  We do this by choosing the inititial consumption value that sets final assets to zero for each household.

	Then we use a second fsolve to find the values of $\left\{ \hat k_{it}, \hat k^f_{it}, \hat n_{ijt} \right\}\; \forall i,j,t$, that make equations \eqref{eq_5ksum}, \eqref{eq_5nsum} and \eqref{eq_5requal} true, subject to the constraint from \eqref{eq_5kfsum}.
