...
\section{Add Demographics and Growth}
	...
	Finally we need to recognize that with mortality, agents will discount future utility not only because of time (via the parameter $\beta$), but also in proportion to the probability of survival.  This is shown below.

    \begin{equation}
		\max_{\{c_{i,s+j,t+j}\}_{j=0}^{S-s}} U_{ist} = \sum_{j=0}^{S-s} \beta^j \rho^c_{ist} \frac{1}{1-\sigma} c_{i,s+j,t+j}^{1-\sigma} \nonumber
	\end{equation}
	where $\rho^c_{ist}$ is the cumulative probability of surviving to age $s$ in country $i$ in period $t$.

	This cummulative probability is given by the following equation.
	\begin{equation}
		\rho^c_{i,s+J,t+J} = \prod_{j=1}^J(1-\rho_{i,s+j,t+j})
	\end{equation}

	When we solve the household's problem we will end up now with a term involving the ratio of $\rho^c_{i,s+J+1,t+J+1}$ to $\rho^c_{i,s+J,t+J}$, which by the equation above will be equal to $1-\rho_{i,s+J+1,t+J+1}$.  Hence in our intertemporal Euler equation we will replace $\beta$ with $\beta (1-\rho_{i,s+1,t+1})$, as shown below.

	\begin{equation}
		c_{ist}^{-\sigma} = \beta (1-\rho_{i,s+1,t+1}) c_{i,s+1,t+1}^{-\sigma}(1+r_{i,t+1}-\delta)
	\end{equation}

	The dynamic system of equations is:
	...
	\begin{align}
		...
		\hat c_{ist}^{-\sigma} & = \beta (1-\rho_{i,s+1,t+1}) \left(\hat c_{i,s+1,t+1} e^{g^A}\right)^{-\sigma}(1+r_{1,t+1}-\delta); \forall i,s \\ 
		& r_{it} - r_{1t} = 0; \forall i>1 \\
...
\section{Add Children and Leisure Decision}
	...
	\begin{align}
       ...
       & \Gamma_{i,t+j} \equiv \left\{ \left[1 + \chi \left(\frac{\chi}{w_{i,t+j} e_{s+j,t+j}}\right)^\rho\right]^{\tfrac{1-\rho \sigma}{\rho}} \right\}^{-\tfrac{1}{\sigma}} \label{eq_3Gamdef}
	\end{align}
	...
	\begin{align}
    	...
    	\hat c_{ist} & = \frac{w_{it} e_{st} \bar \ell_t + (1+r_{it}-\delta)\hat a_{ist} + \hat{bq}_{ist} - \hat a_{i,s+1,t+1} e^{g^A}} {1 + KID_{ist}\Gamma_{it} + \left(\tfrac{\chi}{w_{it}e_{st}}\right)^\rho}; \forall i,s \\
		\hat \ell_{ist} & = \hat c_{ist} \left(\frac{\chi}{w_{it}e_{st}}\right)^\rho ; \forall i,s \\
		\hat c^K_{ist} & = \hat c_{ist} \Gamma_{it}; \forall i,s \\
		\left({\hat c^K_{ist}}\right)^{-\sigma} & = \beta (1-\rho_{i,s+1,t+1}) \left(\hat c^K_{i,s+1,t+1} e^{g^A}\right)^{-\sigma}(1+r_{1,t+1}-\delta); \forall i,s \\
		& r_{it} - r_{1t} = 0; \forall i>1 \\
		& \sum_{i=1}^I \hat k^f_{it} \left( \sum_{s=1}^S \hat N_{ist} \right) = 0
	\end{align}
...
\section{Adding Different Types of Labor}
	...
	\begin{align}
    	...
    	\hat c_{ijst} & = \frac{w_{ijt} e_{jst} \bar \ell_t + (1+r_{it}-\delta)\hat a_{ijst} + \hat{bq}_{jist} - \hat a_{i,j,s+1,t+1} e^{g^A}} {1 + KID_{ist}\Gamma_{ijt} + \left(\tfrac{\chi}{w_{ijt}e_{jst}}\right)^\rho}; \forall i,j,s \\
		...
		\left({\hat c^K_{ijst}}\right)^{-\sigma} & = \beta (1-\rho_{i,s+1,t+1}) \left(\hat c^K_{i,j,s+1,t+1} e^{g^A}\right)^{-\sigma}(1+r_{1,t+1}-\delta); \forall i,j,s \\
		& r_{it} - r_{1t} = 0; \forall i>1 \\
...
\section{Adding Corporate Taxes}
	...
	\begin{align}
		y_{it} & = k_{it}^\alpha \left( A_{i} n_{it} \right)^{1-\alpha} \nonumber \\
		r_{it} & = (1-\tau^K_{it})\alpha \frac{y_{it}}{k_{it}} + \delta \tau^K_{it} \label{eq_5rdef} \\
		w_{it} & = (1-\alpha) \frac{y_{it}}{n_{it}} \nonumber
	\end{align}
	Only \eqref{eq_5rdef} is different than what we have in previous sections.
	...
	\begin{align}
    	...
    	\hat c_{ijst} & = \frac{w_{ijt} e_{jst} \bar \ell_t + (1+r_{it}-\delta)\hat a_{ijst} + \hat{bq}_{ijst} - \hat a_{i,j,s+1,t+1} e^{g^A} + \hat T^L_{it}} {1 + KID_{ist}\Gamma_{ijt} + \left(\tfrac{\chi}{w_{ijt}e_{jst}}\right)^\rho}; \forall i,s \\
		\hat \ell_{ijst} & = \hat c_{ijst} \left(\frac{\chi}{w_{ijt}e_{jst}}\right)^\rho ; \forall i,j,s \\
		\hat c^K_{ijst} & = \hat c_{ijst} \Gamma_{ijt}; \forall i,j,s \\
		\left({\hat c^K_{ijst}}\right)^{-\sigma} & = \beta (1-\rho_{i,j,s+1,t+1}) \left(\hat c^K_{i,j,s+1,t+1} e^{g^A}\right)^{-\sigma}(1+r_{1,t+1}-\delta); \forall i,j,s \\
		& r_{it} - r_{1t} = 0; \forall i>1 \\
		& \sum_{i=1}^I \hat k^f_{it} \left( \sum_{j=1}^J \sum_{s=1}^S \hat N_{ijst} \right) = 0
	\end{align}
